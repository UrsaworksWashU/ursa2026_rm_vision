...
// Same table as SP_CRC16_TABLE in Infantry_Bottom master_process.c
...
uint16_t Get_CRC16_Check_Sum(const uint8_t * msg, uint32_t len, uint16_t init)
{
  uint16_t crc = init;
  while (len--) {
    crc = (crc >> 8) ^ W_CRC_TABLE[(crc ^ *msg++) & 0x00ff];
  }
  return crc;
}
...
uint32_t Verify_CRC16_Check_Sum(const uint8_t * msg, uint32_t len)
{
  ...
  uint16_t expected = Get_CRC16_Check_Sum(msg, len - 2, CRC16_INIT);
...
{
  if (!msg || len <= 2) return;
  uint16_t crc = Get_CRC16_Check_Sum(msg, len - 2, CRC16_INIT);
  msg[len - 2] = static_cast<uint8_t>(crc & 0xff);
  msg[len - 1] = static_cast<uint8_t>((crc >> 8) & 0xff);
}
